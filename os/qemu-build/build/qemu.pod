...
=item B<-netdev vhost-user,chardev=>I<id>B<[,vhostforce=on|off]>
...
=item B<-chardev socket ,id=>I<id> B<[>I<TCP options> B<or> I<unix options>B<] [,server] [,nowait] [,telnet] [,reconnect=>I<seconds>B<]>
...
=item B<HTTP/HTTPS/FTP/FTPS/TFTP>
...
Add named fw_cfg entry from file. I<name> determines the name of
...
=item B<-icount [shift=>I<N>B<|auto]>
...
speed unless B<sleep=no> is specified.
With B<sleep=no>, the virtual time will jump to the next timer deadline
...
B<align=on> will activate the delay algorithm which will try to
